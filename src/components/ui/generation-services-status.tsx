...
import { useModelUsageStatsForDateRange } from "@/hooks/use-analytics";
...
export const GenerationServicesStatus = memo(() => {
  // Get real-time analytics data for all time (since the data is from future timestamps)
  // Use static values to prevent infinite re-renders
  const startOfTime = 0; // Start from beginning of time to capture all data
  const endOfTime = 2000000000000; // Far future timestamp to capture all data

  const modelAnalytics = useModelUsageStatsForDateRange(startOfTime, endOfTime); // Get model stats for all time
  ...
  const totals = useMemo(() => {
    ...
    const pending_requests = 0; // We don't have pending data in model breakdown
  ...
  // Calculate service health and metrics
  const statusItems = useMemo((): ServiceStatusItem[] => {
    if (!modelAnalytics || !totals) {
      // Return loading state with default values
      return [
        {
